...
{int i,read,s;
 ...
 if (waveBPS==16)
    {if (waveChannels==2)
	{read=fread(waveBuff,4,amount,waveFile);
	 writeShort(read);
	 ...
	 /*
	 for (i=0;i<read*4;i+=4)
	    {s=((waveBuff[i]+(waveBuff[i+1]<<8))+
		(waveBuff[i+2]+(waveBuff[i+3]<<8)))>>1;
	     writeShort(s);
	    } */
	}
     else
	{read=fread(waveBuff,2,amount,waveFile);
	 writeShort(read);
    ...
    {read=fread(waveBuff,1,amount,waveFile);
     assert(waveChannels==1);
     writeShort(read);
     for (i=0;i<read;i++)
...
/* if (!lastFrameName || strcmp(lastFrameName,bmpName))*/
 ...
 writeShort(MAXRUN+1); /* write frame termination character */
