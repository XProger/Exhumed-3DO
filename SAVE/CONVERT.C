...
enum {WT_NORM,WT_LAVA,WT_SWAMP,WT_SHOOTER1,WT_SHOOTER2,WT_SHOOTER3,
	WT_SW1,WT_SW2,WT_SW3,WT_SW4,WT_NMTYPES};
...
{int s,x,y,size,dim;
 ...
 if (picWidth<=32 && picHeight<=32 && 
     (tiles[tileNm].flags & TILEFLAG_16BPP))
...
{int psxtileval;
 ...
 psxtileval=(light*21)/128;
 sat=psxtileval-21+16;
 if (sat<0) sat=0;
 if (sat>31) sat=31;
...
{int vrot;
 ...
 if (wall->firstface>=0)
    {int i;
     if (wallTileType[face[wall->firstface].tile]==WT_LAVA)
	outWalls[nmWalls].flags|=WALLFLAG_LAVA;
     if (wallTileType[face[wall->firstface].tile]==WT_SWAMP)
	outWalls[nmWalls].flags|=WALLFLAG_SWAMP;
	...
	{int type=-1;
	 ...
	 if (type!=-1)
	    {object_addObject(type);
	     object_addShort(sectorNm);
	     object_addShort(nmWalls);	     
	     /* tile's center */
	     {int x,y,z;
	     ...
	     /* add normal */
	     object_addInt(outWalls[nmWalls].normal[0]);
	     object_addInt(outWalls[nmWalls].normal[1]);
	     object_addInt(outWalls[nmWalls].normal[2]);	      
	     continue;
	    }
	 ...
	 if (type!=-1)
	    {printf("Found switch on wall %d\n",nmWalls);
	     object_addObject(type);
	     object_addShort(sectorNm);	     
	     if (sectorprops[sectorNm].type==SR_SPECIAL_ROLE_11)
		object_addShort(sectorprops[sectorNm].index); /* channel */
	     else
		object_addShort(1);
	     {int x,y,z;
	      findFaceCenter(i,&x,&y,&z);
...
{int s,s1;
 ...
 for (w=0;w<nmWalls;w++)
	...
	continue;
     if (outWalls[w].normal[1]!=0)
	continue;
     if (outWalls[w].flags & WALLFLAG_WATERBNDRY)
	continue;
...
int pointIsInSector(fVert *p,int s)
{int w;
 fVert testVert,normal;
 for (w=outSector[s].firstWall;w<=outSector[w].lastWall;w++)
    {sWallType *testWall=outWalls+w;
     getVertex(outVert+testWall->v[0],&testVert);
     normal.x=testWall->normal[0]/65536.0;
     normal.y=testWall->normal[1]/65536.0;
     normal.z=testWall->normal[2]/65536.0;
     if ((p->x-testVert.x)*normal.x+
	 (p->y-testVert.y)*normal.y+
	 (p->z-testVert.z)*normal.z<-1.0)
	return 0;
    }
 return 1;
}
...
{int i,type,sec;
 for (i=0;i<nmJeffMonsters;i++)
    {switch (jeffMonster[i].type)
	{case FISH_TYPE:
	    outSector[jeffMonster[i].s].flags|=SECFLAG_WATER;
	    levelHasWater=1;
	    continue;
	   }
	...
	{printf("Unknown object type in jeff file (%d)!\007\n",
     ...
     {fVert p;
      ...
      /* if (pointIsInSector(&p,jeffMonster[i].s)) */
	 sec=jeffMonster[i].s;
      /* else
	 {printf("Jeff monster in wrong sector\007\n");
	  for (s=0;s<nmSectors;s++)
	     if (pointIsInSector(&p,s))
		{sec=s;
		 break;
		}
	  if (s==nmSectors)
	     {printf("Jeff monster not in world!!!!\007");
	      continue;
	     }
	 } */
     }
     if (levelHasRamses && (jeffMonsterMap[type].me==OT_MUMMY || 
			    jeffMonsterMap[type].me==OT_TORCH19))
	{if (jeffMonsterMap[type].me==OT_MUMMY)
	    ram_homeSector=sec;
	 if (jeffMonsterMap[type].me==OT_TORCH19)
	    ram_triggerSector=sec;
...
{int s,w,i;
 ...
 for (i=0;i<monstercount;i++)
    {object_addSpriteObject(monster[i].type,monster[i].s);
    }
#endif
 addJeffObjects();
 addExtraObjects(); 
 /* hack to make all boundries into force fields */
#if 0
 for (w=0;w<nmWalls;w++)
    if (outWalls[w].nextSector!=-1)
       makeForceField(w);
 ...
 /* hack in mobility */
 for (i=0;i<=11;i++)
    outWalls[i].flags|=WALLFLAG_MOBILE;
 for (i=0;i<=11;i++)
    outWalls[i].flags|=WALLFLAG_NOTONCONVEXHULL;
 for (i=18;i<wallcount;i++)
    outWalls[i].flags|=WALLFLAG_MOBILE;
#endif
...
#if 0
 printf("Intersect\n");
 checkForIntersectingSectors();
#endif
 printf("Finding cut planes.\n");
 findCutPlanes(); 
 printf("Done.\n");
...
{int s,w;
 ...
 for (s=0;s<nmWalls;s++)
    {writeInt(outWalls[s].normal[0]);
     ...
     writeInt((int)outWalls[s].object);
...
{int mate,i,bestDistance,s1,w,w1;
 ...
 if (sectorprops[mate].type==SR_SPECIAL_ROLE_8)
    {object_addShort(fl);
     object_addShort(sectorprops[mate].index); /* channel */
    }
 }
 /* add floor */
 {for (w=outSector[s].firstWall;w<=outSector[s].lastWall;w++)
...
{int w,w1;
 ...
 for (s1=0;s1<nmSectors;s1++)
	...
	if (outWalls[w].normal[1]==F(1))
	   {printf("Adjoining sector with floor @ %d\n",
		   outVert[outWalls[w].v[0]].y);
...
if (sectorprops[s].type==SR_LIFT_SHAFT_STYLE_3)
 ...
 for (w=outSector[s].firstWall;w<=outSector[s].lastWall;w++)
	...
	if (!(outWalls[w].flags & WALLFLAG_PARALLELOGRAM))
	   {printf("Elevator floor not rect in sector %d\n",s);
...
int loadSequenceSet(char *filename,int bpp16)
{int i,c,s;
 FILE *ifile;
 char buff[160];
 char path[160];
 ...
 *p=0;
 ...
 for (i=0;i<nmTiles;i++)
	...
	{printf("Tile %s not used\n",tileNames[i]);
     ...
     if (!bpp16)
	tileMap[i]=addTile(buff2,
			   TILEFLAG_64x64|TILEFLAG_8BPP|TILEFLAG_PALLETE);
     else
	tileMap[i]=addTile(buff2,
			   TILEFLAG_64x64|TILEFLAG_16BPP|TILEFLAG_PALLETE);
    }
...
{int o,size,i;
 ...
 {int i,j;
  ...
  for (i=0;i<nmWallTileMaps;i++)
	 ...
	 {if (!stricmp(translateWallTileNm(i),animMap[j].tilename))
		...
		loadSequenceSet(animMap[j].seqName,1);
	 }
     }
 ...
 outSequenceMap[OT_INVISIBLEBALL]=outSequenceMap[OT_AMMOBALL];
 outSequenceMap[OT_WEAPONPOWERBALL]=outSequenceMap[OT_HEALTHBALL];

 outSequenceMap[OT_COBRA]=loadSequenceSet("cobball.seq",0);
 outSequenceMap[OT_POOF]=loadSequenceSet("boom1.seq",0);
...
/* outSequenceMap[OT_RACLOUD]=loadSequenceSet("eyehit.seq",0); */
 ...
 for (o=0;o<nmObjects;o++)
	...
	{case OT_SHOOTER1:
	    ...
	    break;
	 case OT_FLOORSWITCH:
	    outSequenceMap[OT_FLOORSWITCH]=0;
	    outSoundMap[outObjects[o].type]=nmSounds;
	 ...
	 case OT_TORCH19:
	    outSequenceMap[OT_TORCH19]=
	       loadSequenceSet("torches\\mumy.seq",0);
	    break;
	 case OT_TORCH20:
