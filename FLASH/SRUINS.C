/* demo: remove sega_sys from psyq library list */

/* stuff to do:

     save on exit from every level, replace time/date stamp in game listings
        with name of last level exited from
     rewrite rest of render loop in asm
     yuill movie bug (Not his fault! Bad spec give bad results!)
     make movie text background be dark blue w/border
     selkis improvments (larva,charge)
     ...
     secret push door
     light fish
     if object is behind a wall in the initial pass and it does not move
       to a new sector, then smack it into a wall that it was behind
 */
...
static int weaponPowerUpCounter=0;
...
{if (lava)
    {if (ltHurtTime==0)
	{playSound(69,level_staticSoundMap[ST_FLAMER]+0);
	 playSound(69,level_staticSoundMap[ST_FLAMER]+2);
	}
...
{switch (objectType)
	...
	{int diff;
	 ...
	 if (currentState.weaponAmmo[weapon]==weaponMaxAmmo[weapon])
	    return 0;
	 diff=(weaponMaxAmmo[weapon]*(objectType==OT_AMMOBALL?25:50))/100;
	 if (diff<1) diff=1;
...
#define S 10
 ...
 while(1)
	...
	{smoothVTime=vtimer-1;
     ...
     inputEnd=inputQHead;
     totalTime+=vtimer; /* demo */
     lastInputTime+=vtimer; /* demo */
     vtimer=0;
     ENABLE;

     /* demo */
     if ((lastInputSample & (PER_DGT_U|PER_DGT_D|PER_DGT_L|PER_DGT_R))!=
	 (PER_DGT_U|PER_DGT_D|PER_DGT_L|PER_DGT_R))
	lastInputTime=0; 
	...
	{enablePlax(0);
     ...
     /* demo */
     if (totalTime>5*60*FRAMESPERSEC || lastInputTime>30*FRAMESPERSEC)
	{enablePlax(0);
	 return 2;
	}
...
/* #ifndef TESTCODE
 ...
 playIntro(); 
 bup_initCurrentGame();
 currentState.inventory=INV_SANDALS|INV_MASK|INV_SWORD|INV_PISTOL|
    INV_M60|INV_GRENADE;/* demo */
...
/* #endif */
 ...
 while (1)
    {int action;
     ...
     /* demo */
     dlg_flashMessage("-- Exhumed --","Available soon",
		      150,50);
     delay(60*4);
     SYS_Exit(0);
	...
	{case 100 ... 199: /* camel */
	 ...
	 case 3: /* warp to tomb */
	    ...
	    bup_saveGame();
	    level=3;
	    if (currentState.inventory & INV_MUMMY)
	       level=30;
	    levelFile=getLevelName(level);
	    break;
	 case 4: /* good end */
