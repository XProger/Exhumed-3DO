	 ...
	 case IT_GAMEBUTTON:
		...
		{int color=MTH_Sin(waveCycle)>>13;
		 ...
		 drawStringGouro(f(dlgItem[i].xp),f(dlgItem[i].yp),2,
				 greyTable[15+color],greyTable[15-color],
				 buff1);
		 drawStringGouro(f(dlgItem[i].xp)+90,f(dlgItem[i].yp),2,
				 greyTable[15+color],greyTable[15-color],
				 buff2);
		}
	     else
		{drawString(f(dlgItem[i].xp),f(dlgItem[i].yp),2,
			    buff1);
		 drawString(f(dlgItem[i].xp)+90,f(dlgItem[i].yp),2,
			    buff2);
		}
		 ...
		 {pos.y=f(dlgItem[i].yp)+yo[a];
		  /* if ((stat->inventory>>a)&1) */
		     EZ_normSpr(0,COLOR_5,0,mapPic(a+6),&pos,NULL);
		  /* else
		     EZ_normSpr(0,DRAW_GOURAU|COMPO_HARF|COLOR_5,0,
				mapPic(a),&pos,&tbl); */
		  pos.x+=xw[a];
		 }
	     }
...
{int bw=80;
 ...
 dlg_clear();
 w=(w+7)&(~7);
 textHeight=drawCenteredText(0,0,w-30,message,DM_NODRAW,1);
 h=textHeight+bh+40;
...
static int inventoryPicStart[NMINVBUTTONS]={28,14,22,30,39};
...
{XyInt p;
 ...
 commScreenSize=(*(int *)picDatas[38]) * *(((int *)picDatas[38])+1);
 if (fade)
 ...
 while (1)
	...
	{int p,px=0,py=0;
	 if (selectedButton!=3)
	    {/* do normal pictures */
	     ...
	     px=58-((*(int *)picDatas[p])>>1);
	     py=-37-((*(((int *)picDatas[p])+1))>>1);
	     if (selectedButton!=4)
		...
		{{20,23},{-30,-0},{-40,-30},{-50,-10},
	     ...
	     for (i=0;i<8;i++)
		{if (inv&1)
		    {plotOverPic(58+offsets[i][0],-37+offsets[i][1],
				 picStart+i);
		    }
		 else
		    {plotOverPicHarf(58+offsets[i][0],-37+offsets[i][1],
				     picStart+i);
		    }
		 inv>>=1;
		}
		...
		{unsigned short *pallete=picPals[38];
		 if (staticCount)
			...
			{unsigned short color=
			    pallete[((unsigned char *)(picDatas[38]))
				    [i+8]];
			 if (color)
			    POKE_W(VRAMSTART+picVram[38]+(i<<1),
			...
			{unsigned short color=
			    pallete[((unsigned char *)(picDatas[38]))
				    [i+8]];
			 POKE_W(VRAMSTART+picVram[38]+(i<<1),color);
			}
		    }
		 plotOverPic(58+offsets[6][0],-37+offsets[6][1],
			     picStart+8);
		}
	     if (fade)
		{px=58+offsets[fadeSelection][0];
		 py=-37+offsets[fadeSelection][1];
		}
	    }
	 if (fade)
	...
	displayEnable(1);
     noInputTimer++;
     if (noInputTimer>30*FRAMESPERSEC)
	{extern int quitRequest;
	 quitRequest=1;
	 stopAllSound(54);
	 return;
