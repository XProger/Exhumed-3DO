...
{int steps;
 ...
 if (argc<4)
    {printf("Usage: mklseq.exe voiceFile processedFile outFile\n");
     exit(0);
    }
 ...
 for (i=0;i<steps;i++)
    {if (stereo)
	{for (j=i*SAMPLESPERSTEP;j<(i+1)*SAMPLESPERSTEP;j++)
	    {fwrite(waveBuff+(j*4+1),1,1,ofile);
	     fwrite(waveBuff+(j*4+0),1,1,ofile);
	    }
	 for (j=i*SAMPLESPERSTEP;j<(i+1)*SAMPLESPERSTEP;j++)
	    {fwrite(waveBuff+(j*4+3),1,1,ofile);
	     fwrite(waveBuff+(j*4+2),1,1,ofile);
	    }
	}
     else
	for (j=i*SAMPLESPERSTEP;j<(i+1)*SAMPLESPERSTEP;j++)
