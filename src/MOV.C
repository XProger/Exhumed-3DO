...
#define ALLKEYS (PER_DGT_A)
    ...
    { /* decode next frame in buffer */
        if (frame <= 60)
            SCL_SetColOffset(SCL_OFFSET_A, SCL_SP0 | SCL_NBG0 | SCL_RBG0, 4 * (frame - 60), 4 * (frame - 60), 4 * (frame - 60));
        vtimer = 0;
        screenPos = 0;
...
#ifndef NDEBUG
        {
            ...
            parms[1].x = 0;
            parms[1].y = 230;
            parms[2].x = fullBuffers >> 2;
            parms[2].y = 230;
            parms[3].x = fullBuffers >> 2;
            parms[3].y = 220;
        ...
        while ((vtimer < FRAMESKIP - 1 || (0x3ff & PEEK_W(SCL_VDP2_VRAM + 0x18000a)) < 150) && maybeReadBuffer())
            PEEK_W(SCL_VDP2_VRAM + 0x180002);
#else
        while (maybeReadBuffer())
            ;
