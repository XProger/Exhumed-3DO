...
void stepColorOffset(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        else if (colorOffset[i] + colorStepRate < colorCenter[i])
            colorOffset[i] += colorStepRate;
        else
        {
...
static void stepPlayerHeight(void)
{
    if (playerIsDead)
    {
        if (playerHeightOffset > F(31) && abs(playerHeightVel) < 1 << 15)
            return;
    ...
    if (abs(playerHeightVel) < 1 << 13 && abs(playerHeightOffset) < 1 << 13)
    {
        playerHeightVel = 0;
...
void playerDYChange(fix32 dvel)
{
    ...
    if (dvel > F(15))
    {
        ouch = currentState.nmBowls * f(MTH_Mul(dvel - F(12), dvel - F(12)));
        playerHurt(ouch);
    }
...
void underWaterControl(uint16 input)
{
    ...
    else if (!(input & IMASK(ACTION_STRAFE)))
    {
    ...
    else if (!(input & IMASK(ACTION_RUN)))
    {
        force.x += (MTH_Cos(dir) * 60) >> 2;
        force.z += (MTH_Sin(dir) * 60) >> 2;
    }
    ...
    {
        ray.x = -MTH_Sin(playerAngle.yaw);
        ray.y = 0;
        ray.z = MTH_Cos(playerAngle.yaw);
...
void controlInput(uint16 input, uint16 changeInput)
{
    ...
    if (((input & IMASK(ACTION_JUMP)) == 0) && !(camera->flags & SPRITEFLAG_ONSLIPPERYSLOPE))
    {
        camera->vel.y += 3 << 12;
    ...
    if (shawlActive)
    {
        ...
        else if (camera->vel.y < -4 << 15)
            camera->vel.y = -4 << 15;
    }
    else if (camera->vel.y < F(-80))
    {
        camera->vel.y = F(-80);
    }
        ...
        if (!playerMotionEnable || stunCounter)
        {
            ...
            if (playerIsDead)
            {
                ...
                if (playerAngle.pitch < F(80))
                {
                    ...
                    if (playerAngle.pitch > F(90))
                        playerAngle.pitch = F(90);
                    if (playerAngle.pitch < F(-90))
                        playerAngle.pitch = F(-90);
                }
                else
            ...
            if (camera->flags & SPRITEFLAG_UNDERWATER)
            {
                ...
                { /* decide which sector to put bubble in */
                    ...
                    for (i = 0; i < 10; i++)
                    {
                        ...
                        {
                            k++;
                            if (k > level_sector[s].lastWall || level_wall[k].nextSector == -1)
                                k = level_sector[s].firstWall;
                        }
                        if (level_sector[level_wall[k].nextSector].flags & SECFLAG_WATER)
...
void drawStatBar(sint32 time)
{
    ...
    if (weaponMaxAmmo[weapon] && currentState.weaponAmmo[weapon])
    {
        ...
        ammoBar[1].x = -105;
        ammoBar[1].y = 99;
        ammoBar[2].x = -105 + ammoPos;
        ammoBar[2].y = 99;
        ammoBar[3].x = -105 + ammoPos;
        ammoBar[3].y = 95;
...
#ifndef JAPAN
    drawStringGouro(messageXPos, -100, MESSAGEFONT, RGB(t, t, t), RGB(b, b, b), currentMessage);
#else
    {
        char buffer[80];
        sint32 bpos;
        ...
        for (c = currentMessage;; c++)
        {
            ...
            if (*c == '\n' || *c == 0)
            {
                ...
                drawStringGouro(p.x, p.y, 3, RGB(t, t, t), RGB(b, b, b), buffer);
...
void drawAirMeter(sint32 frames)
{
    ...
    switch (meterState)
    {
        ...
        case METERDOWN:
            ...
            else if (!(camera->flags & SPRITEFLAG_UNDERWATER))
            {
...
#if 1
    {
        ...
        pos[1].y = pos[1].x;
        EZ_scaleSpr(ZOOM_MM, DRAW_MESH | COLOR_5 | ECD_DISABLE, 0, mapPic(airBase), pos, NULL);
    }
#else
    pos[0].x = -67;
    pos[0].y = -130 + 120;
    EZ_normSpr(0, UCLPIN_ENABLE | COLOR_5 | ECD_DISABLE, 0, 4, pos, NULL);
    {
        ...
        {
            pos[0].x = f(MTH_Mul(dialLines[c], north.x) + MTH_Mul(dialLines[c + 1], north.y));
            pos[0].y = f(MTH_Mul(dialLines[c], east.x) + MTH_Mul(dialLines[c + 1], east.y));
            pos[1].x = f(MTH_Mul(dialLines[c + 2], north.x) + MTH_Mul(dialLines[c + 3], north.y));
            pos[1].y = f(MTH_Mul(dialLines[c + 2], east.x) + MTH_Mul(dialLines[c + 3], east.y));
            c += 4;
...
sint32 playerGetObject(sint32 objectType)
{
    switch (objectType)
    {
        ...
        case OT_HEALTHSPHERE:
            if (currentState.health == currentState.nmBowls * 200 && airStatus == 0)
                return 0;
            sparkle = 1;
...
#else
            if (currentState.weaponAmmo[currentWeapon] == weaponMaxAmmo[currentWeapon])
                return 0;
            weaponChangeAmmo(currentWeapon, weaponMaxAmmo[currentWeapon] - currentState.weaponAmmo[currentWeapon]);
#endif
 ...
 makeDial();
    ...
    while (1)
    {
        ...
        for (; monsterMoveCounter > 1; monsterMoveCounter -= 2)
        {
            ...
            stepPlayerHeight();
            ouchTime--;
            if (weaponPowerUpCounter && !(currentState.gameFlags & GAMEFLAG_DOLLPOWERMODE))
            {
            ...
            if (invisibleCounter)
            {
                ...
                if (rev < 32)
                {
                    ...
                    if (b > 255)
                        b = 255;
                    SCL_SetColOffset(SCL_OFFSET_B, SCL_NBG0, rg, rg, b);
                }
        ...
        if (mapOn)
            drawMap(camera->pos.x, camera->pos.z, camera->pos.y, playerAngle.yaw, camera->s);
        ...
        drawStringf(-158, -50, 1, "time:%d %d:%d", (lastCalc + lastLastCalc) >> 1, lastDraw, lastCalc + lastDraw);

        drawStringf(-158, -40, 1, "mem:%dk+%dk=%dk", mem_coreleft(0) >> 10, mem_coreleft(1) >> 10, (mem_coreleft(0) + mem_coreleft(1)) >> 10);
        ...
        if (playerMotionEnable && (!(lastInputSample & PER_DGT_S) || !controlerPresent || delayed_fade))
        {
            enablePlax(0);
            runInventory(currentState.inventory, keyMask, &mapOn, delayed_fade, delayed_fadeButton, delayed_fadeSel);
            delayed_fade = 0;
            vtimer = smoothVTime;
        }
...
void main(void)
{
...
#ifdef FLASH
    {
        ...
        {
            bup_initCurrentGame();
            currentState.inventory |= INV_SWORD | INV_PISTOL | INV_M60 | INV_COBRA | INV_SANDALS | INV_MASK;
    ...
    while (1)
    {
        ...
        else if (action >= 200 && action <= 399)
        {
            ...
            if (level == 13 /* kilentry */)
                currentState.inventory |= INV_SANDALS | INV_MASK | INV_SHAWL | INV_ANKLET | INV_SCEPTER | INV_FEATHER;
        }
        else
        {
            switch (action)
