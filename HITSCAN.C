...
int hitSpriteP(Fixed32 ray[3], Fixed32 pos[3], Sprite* sprite, Fixed32* out_pos, Fixed32* out_distance)
{
    ...
    if (v < 0)
        return 0;
    if (f(EO[0]) * f(EO[0]) + f(EO[1]) * f(EO[1]) + f(EO[2]) * f(EO[2]) - f(v) * f(v) > 4 * f(sprite->radius2))
        return 0;
...
int hitWallP(Fixed32 ray[3], Fixed32 pos[3], sWallType* wall, Fixed32* out_pos)
{
    ...
    for (p1 = 0; p1 < 4; p1++)
    {
        ...
        if (p2 == 4)
            p2 = 0;
        if ((p[1] <= V[p1][1] && p[1] <= V[p2][1]) || (p[1] > V[p1][1] && p[1] > V[p2][1]))
            /* point is outside y extent of line */
            continue;
        if (V[p2][1] == V[p1][1])
        { /* horizontal edges */
            if ((p[0] > V[p2][0] && p[0] <= V[p1][0]) || (p[0] > V[p1][0] && p[0] <= V[p2][0]))
                inside = !inside;
            continue;
        }
        ...
        x = V[p1][0] + MTH_Mul((V[p2][0] - V[p1][0]), MTH_Div(p[1] - V[p1][1], V[p2][1] - V[p1][1]));
        if (p[0] <= x)
            inside = !inside;
    }
...
int canSee(Sprite* s1, Sprite* s2)
{
    ...
    d2s = f(s1->pos.x - s2->pos.x) * f(s1->pos.x - s2->pos.x) + f(s1->pos.y - s2->pos.y) * f(s1->pos.y - s2->pos.y) + f(s1->pos.z - s2->pos.z) * f(s1->pos.z - s2->pos.z);
    assert(d2s >= 0);
    d2w = f(s1->pos.x - outPos.x) * f(s1->pos.x - outPos.x) + f(s1->pos.y - outPos.y) * f(s1->pos.y - outPos.y) + f(s1->pos.z - outPos.z) * f(s1->pos.z - outPos.z);
    assert(d2w >= 0);
    if (d2s > d2w)
        return 0;
    else
