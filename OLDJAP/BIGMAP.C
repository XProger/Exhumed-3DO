...
static char *codez[NMCODEZ]={"DRLUDRLUDRLDUL", /* enable full health/ammo */
 ...
 while (1)
...
#else
     poly[0].x=160-105; poly[0].y=30;
     poly[1].x=160+105; poly[1].y=30;
     poly[2].x=160+105; poly[2].y=62;
     poly[3].x=160-105; poly[3].y=62;
#endif
     ...
     else
	{
	 {/* do cheats */
	  int i;
#define NMCP 9
	  static unsigned short cp[NMCP]={PER_DGT_U,PER_DGT_D,PER_DGT_L,
					     PER_DGT_R,PER_DGT_X,PER_DGT_Y,
					     PER_DGT_Z,PER_DGT_A,PER_DGT_S};
			 ...
			 {/* code successful */
			  switch (i)
				 ...
				 break;
			      case 1:
				 if (selectedLevel==13)
				    currentState.gameFlags|=
				       GAMEFLAG_KILENTRYCHEATENABLED;
				 break;
			      case 2:
				 cheatMove=1;
				 break;
